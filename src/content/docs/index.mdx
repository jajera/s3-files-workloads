...
import hero from "../../assets/s3-files-landing-hero.png";

<Image
  src={hero}
  alt="Conceptual illustration: object storage in a bucket connected by NFS-style paths to compute workloads"
  widths={[800, 1200]}
  sizes="(min-width: 80rem) 72rem, 100vw"
  loading="eager"
  decoding="async"
  style="width: 100%; max-width: 72rem; height: auto; border-radius: 0.5rem; margin-block: 0 1.25rem; border: 1px solid var(--sl-color-hairline-light);"
/>

**Amazon S3 Files** mounts an S3 bucket as an NFS file system inside EC2, ECS (Fargate and EC2 launch types), and Lambda — giving your applications a familiar POSIX file-system interface backed by durable object storage.
...
- **Terraform** — `terraform apply` examples for EC2, ECS Fargate, ECS on EC2, and Lambda using the [terraform-aws-s3-files](https://github.com/jajera/terraform-aws-s3-files) modules.
...
<Aside type="note">
  The Terraform AWS provider added native S3 Files resources (`aws_s3files_*`) in version **6.40**.
  ECS task definitions that use `volume.s3files_volume_configuration` need provider **>= 6.41**.
</Aside>
...
| ECS      | Fargate, Managed Instances, and **EC2 launch type** (ECS agent >= 1.104 / `ecs.capability.storage.s3-files`) |
...
<CardGrid>
  ...
  <LinkCard
    ...
    description="Provision all S3 Files infrastructure and your compute resource with a single terraform apply. Examples for EC2, ECS Fargate, ECS on EC2, and Lambda."
  />
</CardGrid>
