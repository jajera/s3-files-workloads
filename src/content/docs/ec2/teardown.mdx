...
4. **Delete the compute security group.** <Badge text="Required" variant="tip" size="small" />
   ...
   <Aside type="note">
     If the instance is still terminating and holds an ENI attached to the SG,
     `delete-security-group` will return `DependencyViolation`. Wait for the instance to reach
     `terminated` state (step 3 wait) before retrying.
   </Aside>
