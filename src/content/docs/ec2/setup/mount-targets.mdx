...
<Aside type="note">
  On [EC2 — Launch & install](/s3-files-workloads/ec2/install-client/), the instance subnet can
  differ from **`SUBNET_ID_AZA`** here as long as it stays in the **same Availability Zone** — that
  lets Session Manager use a subnet with internet egress while mount targets remain on private
  subnets.
</Aside>
...
3. **Create a security group for mount targets.** <Badge text="Required" variant="tip" size="small" />
   ...
   <Aside type="note">
     ...
     Groups](/s3-files-workloads/ec2/setup/security-groups/) step after you know the compute
     security group ID.
   </Aside>
