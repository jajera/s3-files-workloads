...
<Aside type="note">
  Lambda functions must be configured in the **same VPC** as the mount targets. The Lambda security
  group (created in [Security Groups](/s3-files-workloads/lambda/setup/security-groups/)) must have
  outbound NFS (port 2049) to the mount target security group.
</Aside>
...
3. **Create a security group for mount targets.** <Badge text="Required" variant="tip" size="small" />
   ...
   <Aside type="note">
     The inbound NFS rule (port 2049) is added in the [Security
     Groups](/s3-files-workloads/lambda/setup/security-groups/) step after you know the compute
     security group ID.
   </Aside>
...
4. **Create a mount target in each AZ.** <Badge text="Required" variant="tip" size="small" />
   ...
   **Second AZ (required if your VPC has a second AZ and you want HA):**
   ...
   <Aside type="note">
     Bash treats **`SUBNET_ID_AZA`** and **`SUBNET_ID_AZa`** as different variables. Use
     **all-uppercase** `AZA` / `AZB` / `AZC` in both `export` and `$SUBNET_ID_…` references, or
     `--subnet-id` can end up empty and the CLI returns `expected one argument`.
   </Aside>
